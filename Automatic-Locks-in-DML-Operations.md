##  Automatic Locks in DML Operations {#GUID-3D57596F-8B73-4C80-8F4D-79A12F781EFD} 
...
Oracle Database automatically obtains row-level and table-level locks on behalf of DML operations. The type of operation determines the locking behavior. [ Table B-1 ](Automatic-Locks-in-DML-Operations.md#GUID-3D57596F-8B73-4C80-8F4D-79A12F781EFD__BABCJIAJ) summarizes the information in this section. 

> **note:** 

The implicit SX locks shown for the DML statements in [ Table B-1 ](Automatic-Locks-in-DML-Operations.md#GUID-3D57596F-8B73-4C80-8F4D-79A12F781EFD__BABCJIAJ) can sometimes be exclusive (X) locks for a short time owing to side effects from constraints. 
...
` UPDATE ` *table* ...  |  Yes  |  SX  |  Y  Foot 1  |  Y  Foot 1  |  N  |  N  |  N   
` MERGE ` ` INTO ` *table* ...  |  Yes  |  SX  |  Y  |  Y  |  N  |  N  |  N   
` DELETE ` ` FROM ` *table* ...  |  Yes  |  SX  |  Y  Foot 1  |  Y  Foot 1  |  N  |  N  |  N   
` SELECT ` ... ` FROM ` *table* ` FOR ` ` UPDATE ` ` OF ` ...  |  Yes  |  SX  |  Y  Foot 1  |  Y  Foot 1  |  N  |  N  |  N   
` LOCK ` ` TABLE ` *table* ` IN ` ...  |  —  |  |  |  |  |  |   
