[Previous](REGR_-Linear-Regression-Functions.html) [Next](REPLACE.html) JavaScript must be enabled to correctly display this content 
...
![Description of remainder.eps follows](https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/img/remainder.gif)[Description of the illustration remainder.eps](https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/img_text/remainder.html)
...
The `MOD` function is similar to `REMAINDER` except that it uses `FLOOR` in its formula, whereas `REMAINDER` uses `ROUND`. Refer to [MOD](MOD.html#GUID-E12A3928-2C50-45B0-B8C3-82432C751B8C). 
...
Using table `float_point_demo`, created for the `TO_BINARY_DOUBLE` "[Examples](TO_BINARY_DOUBLE.html#GUID-0BA2E065-8006-426C-A3CB-1F6B0C8F283C__I1336203)", the following example divides two floating-point numbers and returns the remainder of that operation: 
