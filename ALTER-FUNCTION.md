[Previous](ALTER-FLASHBACK-ARCHIVE.html) [Next](ALTER-HIERARCHY.html) JavaScript must be enabled to correctly display this content 
...
This statement does not change the declaration or definition of an existing function. To redeclare or redefine a function, use the `CREATE` `FUNCTION` statement with the `OR` `REPLACE` clause. See [CREATE FUNCTION](CREATE-FUNCTION.html#GUID-156AEDAC-ADD0-4E46-AA56-6D1F7CA63306). 
