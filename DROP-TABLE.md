[Previous](SQL-Statements-DROP-TABLE-to-LOCK-TABLE.html) [Next](DROP-TABLESPACE.html) JavaScript must be enabled to correctly display this content 
...
![Description of drop_table.eps follows](https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/img/drop_table.gif)[Descriptionof the illustration drop_table.eps](https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/img_text/drop_table.md)
...
[Oracle Database VLDB and Partitioning Guide](https://docs.oracle.com/pls/topic/lookup?ctx=en/database/oracle/oracle-database/23/sqlrf&id=VLDBG00305) for more information on dropping partitioned tables. 

  * For an index-organized table, any mapping tables defined on the index-organized table are dropped.

  * For a domain index, the appropriate drop routines are invoked. Refer to [Oracle Database Data Cartridge Developer's Guide ](https://docs.oracle.com/pls/topic/lookup?ctx=en/database/oracle/oracle-database/23/sqlrf&id=ADDCI4200)for more information on these routines. 
...
To drop a cluster and all its the tables, use the `DROP` `CLUSTER` statement with the `INCLUDING` `TABLES` clause to avoid dropping each table individually. See [DROP CLUSTER](DROP-CLUSTER.html#GUID-531F7DE2-AA2A-400E-BC9A-4CBEEA7B7156). 
...
The following statement drops the `oe.list_customers` table created in "[List Partitioning Example](CREATE-TABLE.html#GUID-F9CE0CC3-13AE-4744-A43C-EAC7A71AAAB6__I2115317)". 
