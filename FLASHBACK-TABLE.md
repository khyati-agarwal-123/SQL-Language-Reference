[Previous](FLASHBACK-DATABASE.html) [Next](GRANT.html) JavaScript must be enabled to correctly display this content 
...
See Also:
  ...
  * To set the `UNDO_RETENTION` initialization parameter, see [Setting the Minimum Undo Retention Period](https://docs.oracle.com/pls/topic/lookup?ctx=en/database/oracle/oracle-database/23/sqlrf&id=ADMIN-GUID-C1272948-3E08-45D9-BD5A-D6461E249EFB)* [FLASHBACK DATABASE](FLASHBACK-DATABASE.html#GUID-BE0ACF9A-BC13-4810-B08B-33326440258B) for information on reverting the entire database to an earlier version 

  * the [flashback_query_clause](SELECT.html#GUID-CFA006CA-6FF1-4972-821E-6996142A51C6__I2112818) of `SELECT` for information on retrieving past data from a table 

  * [Oracle Database Backup and Recovery User's Guide](https://docs.oracle.com/pls/topic/lookup?ctx=en/database/oracle/oracle-database/23/sqlrf&id=BRADV81517) for additional information on using the `FLASHBACK` `TABLE` statement 
...
Row movement must be enabled for all tables in the Flashback list unless you are flashing back the table `TO` `BEFORE` `DROP`. That operation is called a flashback drop operation, and it uses dropped data in the recycle bin rather than undo data. Refer to [row_movement_clause](ALTER-TABLE.html#GUID-552E7373-BF93-477D-9DA3-B2C9386F2877__I2192391) for information on enabling row movement. 
...
![Description of flashback_table.eps follows](https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/img/flashback_table.gif)[Descriptionof the illustration flashback_table.eps](https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/img_text/flashback_table.md)
...
[CREATE RESTORE POINT](CREATE-RESTORE-POINT.html#GUID-AD0FB693-7C28-4908-A870-BA884B320575) for information on creating restore points 
...
See Also:
  ...
  * [Oracle Database Administrator's Guide](https://docs.oracle.com/pls/topic/lookup?ctx=en/database/oracle/oracle-database/23/sqlrf&id=ADMIN01511) for information on the recycle bin and naming conventions for objects in the recycle bin 

  * [PURGE](PURGE.html#GUID-9257F773-E019-4464-80F4-F5AB61D7D9B6) for information on removing objects permanently from the recycle bin 
