[Previous](EXTRACTVALUE.html) [Next](FEATURE_DETAILS.html) JavaScript must be enabled to correctly display this content 
...
![Description of feature_compare.eps follows](https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/img/feature_compare.gif)[Descriptionof the illustration feature_compare.eps](https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/img_text/feature_compare.md)
...
The syntax of the `FEATURE_COMPARE` function can use an optional `GROUPING` hint when scoring a partitioned model. See [GROUPING Hint](Comments.html#GUID-9693C230-2616-4123-A1ED-3C41E9566F7A). 

mining_attribute_clause

The `mining_attribute_clause` identifies the column attributes to use as predictors for scoring. When the function is invoked with the analytic syntax, these predictors are also used for building the transient models. The `mining_attribute_clause` behaves as described for the `PREDICTION` function. See [mining_attribute_clause](PREDICTION.html#GUID-DA66A1C3-BFB2-43A1-A3FF-93D4A3DAB9C6__MINING_ATTRIBUTE_CLAUSE-58BC0E25). 
